...
Control의 `main`은 bootstrap과 서버 lifecycle을 담당하고, Router/middleware는 `routes`, 공유 상태와 pending command registry는 `state`, SQLite schema lifecycle은 `database`에 둔다. Scenario, Artifact, Run, Network Profile과 metric sample의 영속성 SQL은 `repository` 아래의 리소스별 모듈에 두며 handler는 저장 형식이나 `sqlx::Row`를 알지 못해야 한다. Network profile 기반 endpoint/runtime 해석부터 Artifact 전송, online participant 선택, prepare/start 2단계 합의와 rollback, active 등록, pause/resume/stop/finish까지 Run lifecycle 전체는 `orchestration`이 담당한다. Network plan 생성, Agent별 stage/commit, 실패 보상 rollback, teardown과 command timeout·오류 문맥은 `network_orchestration`이 담당한다. 리비전과 프로파일의 prepared 상태는 `repository/network_profiles`에서 하나의 트랜잭션으로 변경한다. Network operation과 Run lifecycle의 구조화 이벤트 저장·조회는 `repository/diagnostics`가 담당하며 원시 payload, PEM과 plan token을 진단 응답에 포함하지 않는다. HTTP handler는 요청 호환 형식 해석과 상태 코드·응답 JSON 구성만 수행한다. Preflight와 실제 Run 준비는 같은 runtime 해석 함수를 사용해야 한다. Run 상태 SQL은 `repository/runs`를 통해서만 변경한다. API 오류는 `error::ApiError`를 통해서만 HTTP 응답으로 변환한다.
