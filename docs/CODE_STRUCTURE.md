...
Agent의 `main`은 control stream과 workload orchestration을 담당한다. artifact chunk 수신과 무결성 검증은 `artifact`, Run별 payload 준비는 `payload`, 누적 카운터·오류 분류·활성 연결 시간가중 집계는 `telemetry`에 둔다. DNS와 source IP bind를 포함한 TCP 연결, HTTP CONNECT, client TLS upgrade는 `connector`가 담당한다. TCP request/response와 capture turn replay는 `workload/tcp`에 두며, protocol workload는 연결 생성 방법을 직접 구현하지 않는다. Linux namespace 변경은 `network`, 인증서 검증·cipher·TLS version·ALPN 정책은 `tls` 모듈에 둔다. client와 responder는 같은 TLS helper를 사용해야 한다.
