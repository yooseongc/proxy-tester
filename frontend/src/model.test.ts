...
describe("closestSample", () => {
  it("matches telemetry by timestamp rather than arrival order", () => {
    expect(closestSample(point(2_000), [point(900), point(2_050), point(3_100)])?.unix_ms).toBe(
      2_050,
    );
  });
  it("rejects a stale peer sample", () =>
    expect(closestSample(point(5_000), [point(1_000)])).toBeUndefined());
});
...
describe("chart model", () => {
  ...
  it("derives counter rates using the real interval and clamps resets", () => {
    const result = buildChartPoints([
      metricPoint(1, 1000, { connections_failed: 10, transaction_errors: 4 }),
      metricPoint(1, 3000, { connections_failed: 14, transaction_errors: 2 }),
    ]);
    expect(result[1].connection_failures_per_sec).toBe(2);
    expect(result[1].http_errors_per_sec).toBe(0);
  });
  it("computes cumulative stage boundaries and exclusion state", () =>
    expect(
      stageBands([
        {
          name: "Ramp",
      ...
      { name: "Ramp", mode: "ramp", start_ms: 0, end_ms: 2000, included: false },
      { name: "Hold", mode: "hold", start_ms: 2000, end_ms: 5000, included: true },
    ]));
  it("formats bandwidth and latency units", () => {
    expect(formatBandwidth(2_500_000)).toBe("2.50 Mbps");
    expect(formatBandwidth(1_500_000_000)).toBe("1.50 Gbps");
    expect(formatLatency(1.236)).toBe("1.24 ms");
  });
  it("uses per-sample peaks from only the latest one-minute ACTIVE window", () => {
    const samples = [
      metricPoint(1, 1_000, { active_connections_max: 99 }),
      metricPoint(1, 61_001, { active_connections_max: 7 }),
      metricPoint(1, 120_000, { active_connections_max: 12 }),
    ];
