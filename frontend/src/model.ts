...
export type Metrics = {
  ...
  tcp_connect_latency_p50_ms: number;
  tcp_connect_latency_p95_ms: number;
  tcp_connect_latency_p99_ms: number;
  ...
  tcp_retransmissions_per_sec: number;
};
...
export type ChartPoint = {
  ...
  target_vu: number;
  active_connections: number;
  client_app_tx: number;
  client_app_rx: number;
  server_app_tx: number;
  server_app_rx: number;
  client_wire_tx: number;
  client_wire_rx: number;
  server_wire_tx: number;
  server_wire_rx: number;
  tcp_p50: number;
  tcp_p95: number;
  tcp_p99: number;
  http_p50: number;
  http_p95: number;
  http_p99: number;
  connection_failures_per_sec: number;
  http_errors_per_sec: number;
  tcp_retransmissions_per_sec: number;
};
...
export function closestSample(reference: Point | undefined, candidates: Point[], maxSkewMs = 1500) {
  ...
  const closest = candidates.reduce<Point | undefined>(
    (best, current) =>
      !best ||
      Math.abs(current.unix_ms - reference.unix_ms) < Math.abs(best.unix_ms - reference.unix_ms)
        ? current
        : best,
    undefined,
  );
  return closest && Math.abs(closest.unix_ms - reference.unix_ms) <= maxSkewMs
    ? closest
    : undefined;
}

const deltaRate = (current: number, previous: number | undefined, intervalMs: number) =>
  previous === undefined || intervalMs <= 0
    ? 0
    : Math.max(0, current - previous) / (intervalMs / 1000);
...
export function buildChartPoints(points: Point[]): ChartPoint[] {
  ...
  return clients.map((client, index) => {
    const previous = clients[index - 1],
      server = closestSample(client, servers);
    const interval = previous ? client.unix_ms - previous.unix_ms : 0;
    return {
      unix_ms: client.unix_ms,
      elapsed_ms: client.elapsed_ms,
      stage_index: client.load_stage_index,
      cps: client.cps,
      tps: client.tps,
      target_vu: client.desired_virtual_clients,
      active_connections: client.active_connections,
      client_app_tx: client.tx_bps,
      client_app_rx: client.rx_bps,
      server_app_tx: server?.tx_bps ?? 0,
      server_app_rx: server?.rx_bps ?? 0,
      client_wire_tx: client.wire_tx_bps,
      client_wire_rx: client.wire_rx_bps,
      server_wire_tx: server?.wire_tx_bps ?? 0,
      server_wire_rx: server?.wire_rx_bps ?? 0,
      tcp_p50: client.tcp_connect_latency_p50_ms,
      tcp_p95: client.tcp_connect_latency_p95_ms,
      tcp_p99: client.tcp_connect_latency_p99_ms,
      http_p50: client.http_latency_p50_ms,
      http_p95: client.http_latency_p95_ms,
      http_p99: client.http_latency_p99_ms,
      connection_failures_per_sec: deltaRate(
        client.connections_failed,
        previous?.connections_failed,
        interval,
      ),
      http_errors_per_sec: deltaRate(
        client.transaction_errors ?? 0,
        previous?.transaction_errors,
        interval,
      ),
...
export function stageBands(stages: LoadStage[]): StageBand[] {
  ...
  return stages.map((stage) => {
    const start = cursor;
    cursor += stage.duration_secs * 1000;
    return {
      name: stage.name,
      mode: stage.mode,
      start_ms: start,
      end_ms: cursor,
      included: stage.include_in_results,
    };
  });
}
...
export function formatBandwidth(value: number) {
  ...
  const [divisor, unit] =
    absolute >= 1e9 ? [1e9, "Gbps"] : absolute >= 1e6 ? [1e6, "Mbps"] : [1e3, "Kbps"];
...
export function recentActiveMaximum(points: Point[], windowMs = 60_000) {
  ...
  return Math.max(
    ...points
      .filter((point) => point.unix_ms > latest.unix_ms - windowMs)
      .map((point) => point.active_connections_max ?? point.active_connections),
  );
}
...
export type Scenario = {
  ...
  name: string;
  path: ScenarioPath;
  protocol: "tcp" | "http1" | "http2";
  virtual_clients: number;
  ...
  request: {
    ...
    keep_alive: boolean;
    transactions_per_connection: number;
    think_time_ms: number;
  };
  http2: { max_concurrent_streams: number };
  tcp: { tx_bytes: number; rx_bytes: number };
  payload_mode: "manual" | "capture_replay";
  capture_artifact_id: string | null;
  request_payload: PayloadProfile;
  response_payload: PayloadProfile;
  tls: {
    enabled: boolean;
    verify_peer: boolean;
    version: "tls12" | "tls13";
...
export type PayloadProfile = {
  kind: "empty" | "fixed" | "text" | "file" | "random";
  size_bytes: number;
  text: string;
  artifact_id: string | null;
  random_format: "binary" | "printable_ascii";
};
