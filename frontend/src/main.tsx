...
const RunHistory = lazy(() =>
  import("./RunHistory").then((module) => ({ default: module.RunHistory })),
);
import {
  closestSample,
  formatBandwidth,
...
function App() {
  ...
  const navigate = (next: Tab) => {
    ...
    patch = (value: Partial<Scenario>) => setScenario((s) => ({ ...s, ...value }));
  ...
  const syncStages = (stages: LoadStage[]) =>
    patch({
      ...
      virtual_clients: Math.max(1, ...stages.map((s) => s.target_virtual_clients)),
    });
  const stagePatch = (index: number, value: Partial<LoadStage>) =>
    syncStages(scenario.load_stages.map((s, i) => (i === index ? { ...s, ...value } : s)));
  ...
  const loadScenario = (id: string) => {
    ...
    setScenario({
      ...
      load_stages: found.load_stages?.length ? found.load_stages : fallback.load_stages,
    });
    setSaveMessage("불러옴");
  };
  const readPem = async (field: "server_cert_pem" | "server_key_pem" | "ca_pem", file?: File) => {
  ...
  const applyPreset = (kind: "cps" | "http1" | "http2" | "bandwidth" | "dlp" | "pcap") => {
    ...
    if (kind === "http2")
      Object.assign(next, {
        ...
        protocol: "http2",
        tls: { ...next.tls, enabled: true },
      });
    if (kind === "bandwidth")
      Object.assign(next, {
        name: "양방향 B/W",
        request_payload: { ...next.request_payload, size_bytes: 1024 * 1024 },
        response_payload: { ...next.response_payload, size_bytes: 1024 * 1024 },
      });
    if (kind === "dlp")
      Object.assign(next, {
        name: "DLP 양방향 문자열",
        request_payload: { ...next.request_payload, kind: "text", text: "DLP request sentinel" },
        response_payload: { ...next.response_payload, kind: "text", text: "DLP response sentinel" },
      });
    if (kind === "pcap")
      Object.assign(next, { name: "PCAP 세션 재현", payload_mode: "capture_replay" });
    setScenario(next);
  };
  ...
  const payloadLabel = (payload: PayloadProfile | null, direction: string) => {
    ...
    const bytes =
      payload.kind === "text" ? new TextEncoder().encode(payload.text).length : payload.size_bytes;
    const kind = {
      empty: "없음",
      fixed: "고정",
      text: "문자열",
  ...
  const captureBlocked =
    scenario.payload_mode === "capture_replay" &&
    (!selectedCapture ||
      (scenario.protocol === "http2"
        ? (selectedCapture.analysis?.http2_flow_count ?? 0) === 0
        ...
        <header className="sticky top-0 z-50 -mx-3 mb-4 border-b border-line bg-canvas/85 px-3 py-3 backdrop-blur-2xl sm:-mx-6 sm:px-6 lg:-mx-10 lg:px-10">
          <div className="mx-auto flex max-w-[1480px] items-center justify-between gap-4">
            <div className="flex min-w-0 items-center gap-3">
              <img src="/favicon.svg" alt="Proxy Tester" className="size-10 rounded-xl shadow-lg" />
              <div className="min-w-0">
                <p className="truncate font-mono text-[9px] font-bold tracking-[.18em] text-signal">
                  NETWORK PERFORMANCE LAB
                </p>
                <h1 className="m-0 text-lg font-extrabold tracking-[-.04em]">Proxy Tester</h1>
              </div>
            </div>
                ...
                >
                  ...
                  <div>
                    ...
                    <small className="block text-[8px] text-dim">
                      {agent.role === 1 ? "CLIENT" : "SERVER"} · {agent.hostname.slice(0, 8)}
                    </small>
                  </div>
                </div>
              ))}
            ...
            <div className="flex items-center gap-2">
              <StatusBadge tone={activeRun ? "live" : "neutral"}>{status}</StatusBadge>
              <Button
                variant="ghost"
                aria-label="테마 전환"
        ...
        {tab === "setup" && (
          <>
              ...
              onPrepared={(profile_revision_id) =>
                patch({
                  path: {
                    kind: "managed_direct",
                    profile_revision_id,
                    server_port: scenario.path.server_port,
                  },
                })
              }
            />
            <Panel className="p-4 sm:p-6">
              ...
              <div aria-label="시험 프리셋" className="mb-4 flex flex-wrap gap-2">
                {(
                  [
                    ["cps", "TCP CPS"],
                    ["http1", "HTTP/1.1 TPS"],
                    ...
                    <details className="rounded-xl border border-dashed border-line p-3">
                      ...
                      <div className="mt-3 space-y-3">
                        <Field label="연결 경로">
                          ...
                          >
                            <option value="managed_direct">관리형 직접 연결</option>
                            <option value="explicit_proxy">명시적 HTTP Proxy</option>
                          </select>
                        </Field>
                        ...
                        <Field label="Server port">
                          <input
                            ...
                            min="1"
                            max="65535"
                            value={scenario.path.server_port}
                            onChange={(e) =>
                              patch({
                                path: {
                                  ...scenario.path,
                                  server_port: Math.max(1, +e.target.value),
                                },
                              })
                            }
                          />
                        ...
                        <div className="grid gap-2 sm:grid-cols-3">
                          <Field label="Connect timeout (ms)">
                            <input
                              type="number"
                              min="1"
                              value={scenario.timeouts.connect_ms}
                              onChange={(e) =>
                                patch({
                                  timeouts: {
                                    ...scenario.timeouts,
                                    connect_ms: Math.max(1, +e.target.value),
                                  },
                                })
                              }
                            />
                          </Field>
                          <Field label="Proxy timeout (ms)">
                            <input
                              type="number"
                              min="1"
                              value={scenario.timeouts.proxy_connect_ms}
                              onChange={(e) =>
                                patch({
                                  timeouts: {
                                    ...scenario.timeouts,
                                    proxy_connect_ms: Math.max(1, +e.target.value),
                                  },
                                })
                              }
                            />
                          ...
                          <Field label="Response timeout (ms)">
                            <input
                              type="number"
                              min="1"
                              value={scenario.timeouts.response_ms}
                              onChange={(e) =>
                                patch({
                                  timeouts: {
                                    ...scenario.timeouts,
                                    response_ms: Math.max(1, +e.target.value),
                                  },
                                })
                              }
                            />
                  ...
                  >
                    <Field label="Payload 모드">
                      <select
                        ...
                        onChange={(e) =>
                          patch({
                            payload_mode: e.target.value as Scenario["payload_mode"],
                          })
                        }
                      >
                    ...
                    {scenario.payload_mode === "manual" ? (
                      <div className="grid gap-3 sm:grid-cols-2">
                        <PayloadEditor
                          ...
                          onChange={(request_payload) => patch({ request_payload })}
                        />
                        <PayloadEditor
                          ...
                          uploading={uploading}
                          onUpload={uploadPayload}
                          onChange={(response_payload) => patch({ response_payload })}
                        />
                      </div>
                    ) : (
                      <CaptureEditor
                        ...
                        onSelect={(capture_artifact_id) => patch({ capture_artifact_id })}
                      />
                    )}{" "}
                    {scenario.protocol === "http2" && (
                      <Field label="연결당 최대 동시 Stream">
                        <input
                          aria-label="연결당 최대 동시 Stream"
                          type="number"
                          min="1"
                          max="1000"
                          value={scenario.http2.max_concurrent_streams}
                          onChange={(e) =>
                            patch({
                              http2: {
                                max_concurrent_streams: Math.max(
                                  1,
                      ...
                      <>
                        <Field label="Connection 사용 방식">
                          ...
                          >
                            ...
                            <option value="fixed">Connection마다 지정 횟수</option>
                            <option value="continuous">Connection 유지 반복</option>
                          </select>
                        </Field>
                        {transactionMode === "fixed" && (
                          <Field label="Connection당 Transaction 수">
                            <input
                              ...
                              onChange={(e) =>
                                requestPatch({
                                  transactions_per_connection: Math.max(2, +e.target.value),
                                })
                              }
                            />
                          </Field>
                          ...
                          <select
                            ...
                            onChange={(e) =>
                              patch({
                                tls: {
                                  ...scenario.tls,
                                  version: e.target.value as Scenario["tls"]["version"],
                                  cipher_suite: null,
                                },
                              })
                            }
                          ...
                          <div className="mt-3 space-y-3">
                            ...
                            <Field label="Server certificate PEM">
                              <input
                                type="file"
                                accept=".pem,.crt"
                                onChange={(e) => readPem("server_cert_pem", e.target.files?.[0])}
                              />
                              <textarea
                                rows={3}
                            ...
                            <Field label="Server private key PEM">
                              <input
                                type="file"
                                accept=".pem,.key"
                                onChange={(e) => readPem("server_key_pem", e.target.files?.[0])}
                              />
                              <textarea
                                rows={3}
                            ...
                            {scenario.tls.verify_peer && (
                              <Field label="CA certificate PEM">
                                <input
                                  type="file"
                                  accept=".pem,.crt"
                                  onChange={(e) => readPem("ca_pem", e.target.files?.[0])}
                                />
                                <textarea
                                  rows={3}
                      ...
                      >
                        <div className="mb-3 flex items-center gap-2">
                          ...
                          <input
                            ...
                            value={stage.name}
                            onChange={(e) => stagePatch(index, { name: e.target.value })}
                          />
                          <Button
                            variant="ghost"
                          ...
                          <Button
                            ...
                            onClick={() =>
                              syncStages(scenario.load_stages.filter((_, i) => i !== index))
                            }
                            aria-label="삭제"
                          >
                            <Trash2 size={13} />
                        ...
                        <div className="grid gap-2 sm:grid-cols-3">
                          ...
                          <Field label="목표 VU">
                            <input
                              ...
                              onChange={(e) =>
                                stagePatch(index, {
                                  target_virtual_clients: Math.max(0, +e.target.value),
                                })
                              }
                            />
                          </Field>
              ...
              {captureBlocked && (
                ...
                >
                  {selectedCapture
                    ? `선택한 capture에 지원 가능한 ${scenario.protocol === "http1" ? "HTTP/1.1 transaction" : "양방향 TCP flow"}이 없어 실행할 수 없습니다.`
          ...
          <Panel className="p-4 sm:p-6">
            ...
            <div className="mb-4 grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
              {[
                ...
                ["목표 VU", latestClient?.desired_virtual_clients ?? scenario.virtual_clients],
                ["Active", latestClient?.active_connections ?? 0],
                ["집계", latestClient?.included_in_results === false ? "제외" : "포함"],
              ].map(([label, value]) => (
                <div key={label} className="rounded-xl border border-line bg-inset px-3 py-2.5">
            ...
            <div className="mb-5 grid grid-cols-2 gap-2 md:grid-cols-4 xl:grid-cols-8">
              <MetricCard label="CPS" value={latestClient?.cps.toFixed(0) ?? "—"} icon={Gauge} />
              <MetricCard
                label="ACTIVE"
                ...
                tone="info"
              />
              <MetricCard label="TPS" value={latestClient?.tps.toFixed(0) ?? "—"} icon={Activity} />
              <MetricCard
                label="HTTP P99"
                value={
              ...
              <MetricCard
                label="CLIENT TX"
                value={latestClient ? formatBandwidth(latestClient.tx_bps) : "—"}
                icon={Radio}
              />
              <MetricCard
                label="SERVER RX"
                value={latestServer ? formatBandwidth(latestServer.rx_bps) : "—"}
                icon={Server}
                tone="violet"
              />
              <MetricCard
                label="SERVER TX"
                value={latestServer ? formatBandwidth(latestServer.tx_bps) : "—"}
                icon={Wifi}
                tone="violet"
              />
              <MetricCard
                label="CLIENT RX"
                value={latestClient ? formatBandwidth(latestClient.rx_bps) : "—"}
                icon={Globe2}
                tone="info"
              />
            </div>
            <Suspense fallback={<p role="status">차트 불러오는 중…</p>}>
              <TelemetryCharts
                points={points}
                scenario={scenario}
                theme={theme}
    ...
    <fieldset className="min-w-0 rounded-xl border border-line bg-inset p-3">
      ...
      <Field label="종류">
        <select
          aria-label={`${label} 종류`}
          value={value.kind}
          onChange={(e) => patch({ kind: e.target.value as PayloadProfile["kind"] })}
        >
      ...
      {(value.kind === "fixed" || value.kind === "random") && (
        <Field label="크기 (bytes)">
          <input
            ...
            onChange={(e) =>
              patch({
                size_bytes: Math.max(0, Math.min(64 * 1024 * 1024, +e.target.value)),
              })
            }
          />
        </Field>
      ...
      {value.kind === "random" && (
        <Field label="Random 형식">
          <select
            ...
            onChange={(e) =>
              patch({
                random_format: e.target.value as PayloadProfile["random_format"],
              })
            }
          >
      ...
      {value.kind === "file" && (
        <>
          ...
          <Field label="Payload artifact">
            <select value={value.artifact_id ?? ""} onChange={(e) => choose(e.target.value)}>
...
function CaptureEditor({
  ...
  const captures = artifacts.filter((a) => a.kind === "pcap"),
    ...
    excluded = Object.entries(current?.analysis?.exclusions ?? {}).filter(([, count]) => count > 0);
  const supported = current
    ? protocol === "http2"
      ? (current.analysis?.http2_flow_count ?? 0)
      : protocol === "http1"
        ? (current.analysis?.http_flow_count ?? 0)
        : (current.analysis?.supported_flow_count ?? 0)
    : 0;
  return (
    <div className="space-y-3 rounded-xl border border-dashed border-line bg-inset p-3">
      ...
      {uploading && (
        <p role="status" className="rounded-lg bg-signal/10 p-2 text-xs font-bold text-signal">
          업로드 및 분석 중…
        </p>
      )}
      <Field label="분석된 Capture">
        <select value={selected ?? ""} onChange={(e) => onSelect(e.target.value || null)}>
          ...
          {captures.map((a) => (
            <option key={a.id} value={a.id}>
              {a.name} · {a.format} · {a.analysis?.supported_flow_count ?? 0} flows ·{" "}
              {a.analysis?.http_transaction_count ?? 0} HTTP tx
            </option>
          ))}
        </select>
      </Field>
      {current && (
        <div className="grid gap-2 text-[10px] sm:grid-cols-2" aria-label="Capture 분석 요약">
          <p
            className={`rounded-lg p-2 sm:col-span-2 ${supported > 0 ? "bg-signal/10 text-signal" : "bg-warn/10 text-warn"}`}
          >
            <b className="block">현재 프로토콜 지원</b>
            {supported}개{" "}
            {protocol === "http2" ? "HTTP/2 흐름" : protocol === "http1" ? "HTTP 흐름" : "TCP 흐름"}
          </p>
          ...
          <p className="rounded-lg bg-raised p-2">
            <b className="block text-signal">HTTP/2</b>
            {current.analysis?.http2_flow_count ?? 0}개 흐름 ·{" "}
            {current.analysis?.http2_transaction_count ?? 0} transactions
          </p>
          <p className="rounded-lg bg-raised p-2">
            <b className="block text-signal">Retransmission</b>
            {(current.analysis?.retransmitted_bytes ?? 0).toLocaleString()} bytes
          </p>
          {excluded.length > 0 && (
            <p className="rounded-lg bg-raised p-2 sm:col-span-2">
              <b className="block text-dim">제외 요약</b>
              {excluded.map(([reason, count]) => `${reason}: ${count}`).join(" · ")}
            </p>
          )}
        </div>
      )}
